...
function ClasiDetailCtrl($scope, $routeParams, $http) {

		/*
		$http.get('/details').success(function(data) {
			//$scope.clasiData = data;
		});
		*/
		
		if($scope.clasiData.property) {
			$scope.name = $scope.clasiData.property;
		}
		else {
			$scope.name = "My Custom House";
		}
